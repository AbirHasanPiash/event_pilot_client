...
export default function AdminEventsPage() {
  ...
  // SWR key
  const swrKey = useMemo(() => `/api/events/?${buildQuery(page)}`, [
    buildQuery,
    page,
  ]);
  ...
  useEffect(() => {
    ...
    const url = `/user/dashboard/admin/events?${qs}`;

    try {
      if (typeof sessionStorage !== "undefined") {
        sessionStorage.setItem("admin_events_last_list_url", url);
      }
    } catch {
      ...
      {events.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between mt-6 gap-3">
          <span className="text-sm text-gray-600">
            Page {page} — Showing <b>{events.length}</b> of <b>{count}</b> events
          </span>
